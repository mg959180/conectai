...
class SettingsController
{
    ...
    public function index()
    {
        if (isset($_POST['save_setting'])) {
            ...
            $sql = "UPDATE " . WEBSITE_SETTINGS . " SET wes_name = :name, wes_email_verification_time = :email_verification_time, wes_otp_attempts = :otp_attempts,
            wes_maintenance_mode = :mode, wes_maintenance_start_time = :s_date, wes_maintenance_end_time = :e_date, wes_mail_active = :mail_status,
            wes_mailer = :mailer, wes_mailer_host = :mail_host, wes_mailer_port = :mail_port, wes_mailer_encryption = :mail_enc, wes_mailer_uname = :mail_u_name,
            wes_mailer_upass = :mail_u_pass, wes_mailer_from_address = :mail_from_address, wes_mailer_from_name = :mail_from_name, wes_open_ai_uid = :open_ai_uid,
            wes_open_ai_upass =:open_ai_upass, wes_open_ai_key = :open_ai_key, wes_demo_websites = :open_ai_website_no, wes_demo_pages = :open_ai_website_no_pages, 
            wes_open_ai_demo_days = :open_ai_demo_days, wes_modified_by = :by, wes_modified_date = :date WHERE wes_id = 1";

            try {
                $this->_db->beginTransaction();
                $this->_db->query($sql);
                $this->_db->bind(":name", $data['name']);
                $this->_db->bind(":email_verification_time", (intval($data['email_verification_time']) * 60));
                $this->_db->bind(":otp_attempts", $data['otp_attempts']);
    ...
    public function paymentInfoSetting()
    {
        if (isset($_POST['save_payment_setting'])) {
            $data = Input::filter_param($_POST);
            $sql = "UPDATE " . WEBSITE_SETTINGS . " SET wes_is_bank_active = :is_bank_active, wes_bank_name = :bank_name, wes_bank_ac_holder_name = :bank_ac_holder_name,
            wes_bank_ac_no = :bank_ac_no, wes_bank_ac_ifcs_no = :bank_ac_ifcs_no, wes_modified_by = :by, wes_modified_date = :date WHERE wes_id = 1";
            try {
                $this->_db->beginTransaction();
                $this->_db->query($sql);
                $this->_db->bind(":is_bank_active", $data['is_bank_active']);
                $this->_db->bind(":bank_name", $data['bank_name']);
                $this->_db->bind(":bank_ac_holder_name", $data['bank_ac_holder_name']);
                $this->_db->bind(":bank_ac_no", $data['bank_ac_no']);
                $this->_db->bind(":bank_ac_ifcs_no", $data['bank_ac_ifcs_no']);
                $this->_db->bind(":by", Session::get('admin')['uId']);
                $this->_db->bind(":date", date(SYSTEM_DATE_TIME_FORMAT_LONG));
                $retVal = $this->_db->execute();
                if ($retVal) {
                    $this->_db->commit();
                    set_session_alert('success', 'Website Payment Info. Settings Updated Successfully');
                    redirect(SITE_ADMIN_URL . 'settings/payment-info-setting');
                }
            } catch (Exception $ex) {
                $this->_db->rollBack();
                set_session_alert('error', $ex->getMessage());
        ...
        $this->_view->setVal('meta_author', 'Mayank Gupta');
        $this->_view->setVal('page_title', 'Payment Info. Setting Page');
        $this->_view->adminPageRender(ADMIN_VIEW_DIR . 'settings/payment-info-setting-form');
    }
}
